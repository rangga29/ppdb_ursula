...
class Tbtk extends BaseController
{
    protected $tbtkModel;
    ...
    public function data_calon_siswa($slug_nama_lengkap)
    {
        ...
        $data = [
            ...
            'tbtk' => $this->tbtkModel->getTbtk($slug_nama_lengkap),
            'dapodik' => $this->tbtkDapodikModel->getTbtk($tbtk_id),
            'pernyataan' => $this->tbtkPernyataanModel->getPernyataan($tbtk_id),
            'beasiswa' => $this->tbtkBeasiswaModel->getBeasiswa($tbtk_id),
            'keuangan' => $this->tbtkKeuanganModel->getKeuangan($tbtk_id),
            'pembayaran_tahap_1' => $this->tbtkPembayaranModel->getPembayaranTahap1($tbtk_id),
            'pembayaran_tahap_2' => $this->tbtkPembayaranModel->getPembayaranTahap2($tbtk_id),
            'pembayaran_tahap_3' => $this->tbtkPembayaranModel->getPembayaranTahap3($tbtk_id),
            'pembayaran_tahap_4' => $this->tbtkPembayaranModel->getPembayaranTahap4($tbtk_id),
		];
		return view('dashboard/tbtk/data_calon_siswa', $data);
    }
    ...
    public function penggantian_password($slug_nama_lengkap)
    {
		...
		return view('dashboard/tbtk/ganti_password', $data);
    }
    ...
    public function change_password($id)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/penggantian_password/' . $this->request->getVar('slug_nama_lengkap'))->withInput();
        };

        $old_password = $this->request->getVar('old_password');
        $new_password = $this->request->getVar('new_password');

        $dataUser = $this->tbtkModel->getSiswaById($id);
    ...
    public function forget_password()
    {
        $data = [
            ...
            'unit' => 'tbtk',
			'slug' => 'tbtk_login',
			'judul1' => 'PPDB ONLINE 2022-2023 <br><br> LUPA PASSWORD AKUN <br> TB-TK SANTA URSULA',
    ...
    public function confirm_data()
    {
        ...
        $dataUser = $this->tbtkModel->getDataChangePassword($no_registrasi, $email);
        ...
        if($dataUser) {
            ...
            return redirect()->to('/tbtk/forget_password')->withInput();
        }
    }
    ...
    public function new_password($no_registrasi)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/confirm_data')->withInput();
        };

        $new_password = $this->request->getVar('new_password');

        $dataUser = $this->tbtkModel->getSiswaByNoRegistrasi($no_registrasi);
        if ($dataUser) {
            $this->tbtkModel->save([
                'id' => $dataUser->id,
                'password' => password_hash($new_password, PASSWORD_BCRYPT),
            ...
            session()->setFlashdata('pesan', 'Password berhasil diubah.');
            return redirect()->to('/tbtk_login')->withInput();
        } else {
    ...
    public function ubah_data_pendaftaran($slug_nama_lengkap)
    {
		...
		return view('dashboard/tbtk/update_pendaftaran', $data);
    }
    ...
    public function update_data_pendaftaran($id)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/ubah_data_pendaftaran/'.$this->request->getVar('slug_nama_lengkap'))->withInput();
        };
        ...
        return redirect()->to('/tbtk/data_calon_siswa/'.$slug_nama_lengkap);
    }
    ...
    {
        $tbtk = $this->tbtkModel->getTbtk($slug_nama_lengkap);
        if($tbtk->status_dapodik != 1) {
            return redirect()->to('dashboard/tbtk/'.$slug_nama_lengkap);
        }
        $data = [
        ...
        return view('dashboard/tbtk/form_data_dapodik', $data);
    }
    ...
    public function tambah_data_dapodik($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/data_dapodik/'.$slug_nama_lengkap)->withInput();
        };
    ...
    public function ubah_data_dapodik($slug_nama_lengkap)
    {
        ...
        $data = [
            ...
            'slug_unit' => 'tbtk',
            'tbtk' => $tbtk,
            'dapodik' => $this->tbtkDapodikModel->getTbtk($tbtk->id),
            'validation' => \Config\Services::validation()
        ];
        return view('dashboard/tbtk/update_data_dapodik', $data);
    }
    ...
    public function update_data_dapodik($id)
    {
        ...
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/ubah_data_dapodik/'.$slug_nama_lengkap)->withInput();
        };
        ...
        if($filePasFoto->getError() == 4) {
            ...
            $namaPasFoto = $filePasFoto->getRandomName();
            $filePasFoto->move('upload/pas_foto/tbtk', $namaPasFoto);
            unlink('upload/pas_foto/tbtk/' . $this->request->getVar('old_pas_foto'));
        }
        ...
        if($fileScanKartuKeluarga->getError() == 4) {
            ...
            $namaScanKartuKeluarga = $fileScanKartuKeluarga->getRandomName();
            $fileScanKartuKeluarga->move('upload/scan_kk/tbtk', $namaScanKartuKeluarga);
            unlink('upload/scan_kk/tbtk/' . $this->request->getVar('old_scan_kk'));
        }
        ...
        if($fileScanAktaKelahiran->getError() == 4) {
            ...
            $namaScanAktaKelahiran = $fileScanAktaKelahiran->getRandomName();
            $fileScanAktaKelahiran->move('upload/scan_ak/tbtk', $namaScanAktaKelahiran);
            unlink('upload/scan_ak/tbtk/' . $this->request->getVar('old_scan_ak'));
        }
    ...
    public function data_pernyataan($slug_nama_lengkap)
    {
        setlocale(LC_ALL, 'id-ID', 'id_ID');
        $tbtk = $this->tbtkModel->getTbtk($slug_nama_lengkap);
        if($tbtk->status_pernyataan != 1) {
            return redirect()->to('dashboard/tbtk/'.$slug_nama_lengkap);
        }
        $tbtk_tanggal_lahir = strtotime($tbtk->tanggal_lahir);
        $tanggal_lahir = strftime("%d %B %Y",$tbtk_tanggal_lahir);
        $data = [
        ...
        return view('dashboard/tbtk/form_data_pernyataan', $data);
    }
    ...
    public function tambah_data_pernyataan($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/data_pernyataan/'.$slug_nama_lengkap)->withInput();
        };
        ...
        $tbtk = $this->tbtkModel->getTbtk($slug_nama_lengkap);
        $folderPath = "upload/tanda_tangan2/tbtk/";
    ...
    public function data_keuangan($slug_nama_lengkap)
    {
        ...
        return view('dashboard/tbtk/form_data_keuangan', $data);
    }
    ...
    public function tambah_data_keuangan($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/data_keuangan/'.$slug_nama_lengkap)->withInput();
        };
        ...
        if($uang_pangkal != $total_uang)
        {
            ...
            $folderPath = "upload/tanda_tangan/tbtk/";
            $image_parts = explode(";base64,", $_POST['tanda_tangan']); 
            $image_type_aux = explode("image/", $image_parts[0]);
            $image_type = $image_type_aux[1];
            $image_base64 = base64_decode($image_parts[1]);
    ...
    public function pembayaran_tahap_1($slug_nama_lengkap)
    {
        ...
        return view('dashboard/tbtk/form_upload_pembayaran', $data);
    }
    ...
    public function bukti_pembayaran_tahap_1($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/pembayaran_tahap_1/'.$slug_nama_lengkap)->withInput();
        };
        ...
        $fileBuktiBayar->move('upload/bukti_tahap_1/tbtk', $namaBuktiBayar);

        $this->tbtkPembayaranModel->save([
            'tbtk_id' => $tbtk->id,
            'tahap' => '1',
    ...
    public function bukti_pembayaran_tahap_2($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/pembayaran_tahap_2/'.$slug_nama_lengkap)->withInput();
        };
        ...
        $namaBuktiBayar = $fileBuktiBayar->getRandomName();
        $fileBuktiBayar->move('upload/bukti_tahap_2/tbtk', $namaBuktiBayar);
    ...
    public function bukti_pembayaran_tahap_3($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/pembayaran_tahap_3/'.$slug_nama_lengkap)->withInput();
        };
        ...
        $namaBuktiBayar = $fileBuktiBayar->getRandomName();
        $fileBuktiBayar->move('upload/bukti_tahap_3/tbtk', $namaBuktiBayar);
    ...
    public function bukti_pembayaran_tahap_4($slug_nama_lengkap)
    {
        if(!$this->validate([
            ...
            return redirect()->to('/tbtk/pembayaran_tahap_4/'.$slug_nama_lengkap)->withInput();
        };
        ...
        $namaBuktiBayar = $fileBuktiBayar->getRandomName();
        $fileBuktiBayar->move('upload/bukti_tahap_4/tbtk', $namaBuktiBayar);
